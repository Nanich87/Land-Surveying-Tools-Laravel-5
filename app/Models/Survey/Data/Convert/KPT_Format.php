...
class KPT_Format implements \App\Contracts\ConvertibleFormat {

    public function convert($data, $format) {
        $method = 'convertTo' . $format;
        if (!method_exists(__CLASS__, $method)) {
            throw new \Exception(sprintf("KPT format does not support conversion to %s!", $format));
        }
    ...
    private function convertToGPX($data) {
        ...
        foreach ($data as $line) {
            $outputString .= '<wpt lat="' . $line['x'] . '" lon="' . $line['y'] . '">';
            $outputString .= ' <name>' . $line['p'] . '</name>';
